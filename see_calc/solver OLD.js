// using sympy, and back_solve and forward_solve to solve system of equations
...
function back_solve(eqns,sub_vars){
    ...
    for (let i=0;i<eqns.length;i++){
        ...
        var new_vars = []
        unordered_eqns = [] // reset unordered_eqns before pushing the simplified expressions
        unordered_eqns_unsimp.forEach((eqn)=>{
            var exp_simp = sympy_simplify(eqn)
            var eqn_vars = get_all_vars([exp_simp])
            if (parseFloat(exp_simp)!==0 && eqn_vars.length===0){
        ...
        for (let j=0;j<unordered_eqns.length;j++){
            unordered_eqns[j] = sub_all_vars(unordered_eqns[j],var_sel,eqn_sol)//nerdamer(unordered_eqns[j]).sub(var_sel,eqn_sol).toString()
        }
...
function forward_solve(ordered_sols,ordered_vars){
    ...
    for (let i=0;i<ordered_sols.length;i++){
        ...
        for (let j=0;j<ordered_sols.length;j++){
            if (i!==j){
                ordered_sols[j] = sub_all_vars(ordered_sols[j],var_sel,sol)//nerdamer(ordered_sols[j]).sub(var_sel,sol).toString()    //! replace with sub_vars
            }
        }
    }
...
function sympy_compute(command_op_specific,eqn) {   
