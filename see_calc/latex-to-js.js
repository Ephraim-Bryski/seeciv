/*
exps.forEach(exp=>{
	console.log(ltx_to_math(exp))
})
*/

function ltx_to_math(eqn){



	var exps = eqn.split("=")
	var math_exps = exps.map((exp)=>{
		try{
			var math_exp =  taken_latex_to_js(exp)
		}catch{
			console.log("latex-to-js couldnt parse")
			throw "cannot parse"
		}

		math_exp = math_exp.replaceAll("pow(","(")
		math_exp = math_exp.replaceAll(",",")^(")
	
		// replace (my_var) with my_var (remove unnecessay parentheses) (messes up trig functions)
		/*
		enclosed =/\(\w+\)/g
		var math_exp = math_exp.replaceAll(enclosed,(exp)=>{
			var enclosed_var = exp.replaceAll("(","").replaceAll(")","")
			return enclosed_var
		})
		*/
	...
	var math_eqn = math_exps.join("=")

	

	return math_eqn

	function taken_latex_to_js(input) {
		// this is the code from latex-to-js without modification
